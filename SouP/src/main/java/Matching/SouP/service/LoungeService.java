...
public class LoungeService {
    ...
    public JSONArray showLoungeForUser(User user) {
        List<Lounge> loungeList = loungeRepository.findAllDesc();
        JSONArray arr = new JSONArray();
        for (Lounge lounge : loungeList) {
            JSONObject obj = makeLoungeForm(lounge);
            if (user.getLoungeConnectList()
                    .size() != 0) {
                boolean isFav = user.getLoungeConnectList().stream()
                        .anyMatch(fav -> fav.getLounge().getId().equals(lounge.getId()));
                obj.put("isfav", isFav);
            }
            arr.add(obj);
        }
